...
const GrafanaUrl = "/grafana/d/Hw7tu7aZz123123/kstone?var-datasource=KSTONE-PROM&theme=light&kiosk=tv&orgId=1&var-job=";
...
export function Monitor(): JSX.Element {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ height: "100%", width: "100%", margin: "0px" }}>{cluster.metadata === undefined || cluster.metadata.labels === undefined || (cluster.metadata.labels.monitor === "false" && cluster.metadata.labels.consistency === "false" && cluster.metadata.labels.healthy === "false" && cluster.metadata.labels.request === "false") ? <>{<Empty style={{ height: "100%" }} description={`${t("Cluster")}${clusterName}${"HasNotYetEnabledTheMonitorFeature"}`} />}</> : <iframe title="etcd monitor" src={grafanaUrl} width="100%" height="100%"></iframe>}</div>
      </Content>
    </Layout>
  );
}
