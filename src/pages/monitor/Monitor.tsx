...
export function Monitor(): JSX.Element {
  ...
  return (<Layout style={{
    ...
    <Header className='site-layout-background' style={{ padding: 0 }}>
      <Text strong style={{ float: 'left', marginLeft: '15px', marginRight: '15px' }}>集群监控</Text>
      <Select
        style={{ marginTop: '5px' }}
        ...
        onChange={(value) => {
          ...
          setCluster(clusterList.filter((item: any) => {
      ...
      <div style={{ height: '100%', width: '100%', margin: '0px' }}>
        {
          cluster.metadata === undefined ||
            ...
            <>
              {
                <Empty style={{ height: '100%' }} description={`集群${clusterName}尚未开启监控功能`} />
              }
            </>) : <iframe title='etcd monitor' src={grafanaUrl} width='100%' height='100%'></iframe>
        }
      </div>
    </Content>
  </Layout>);
